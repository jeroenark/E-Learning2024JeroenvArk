...
function onSubmit() {
    ...
    $username = "klas4s22_575014";

    $password = "WhqM4jJl";

    
    $beschrijving = htmlspecialchars($_POST['beschrijving'], FILTER_SANITIZE_STRING);

    $conn = new mysqli($servername, $username, $password, 'klas4s22_575014');    

    $sql = "INSERT INTO `lijst`(`users_id`, `name`, `description`, `mode_id`) VALUES (?,?,?,?)";

    $stmt = $conn->prepare($sql);

    $stmt->bind_param('issi', $_SESSION['gebruikersID'], htmlspecialchars($_POST['titel']), htmlspecialchars($_POST['beschrijving']), $_POST['mode']);

    $stmt->execute();

    // Redirect to lijst.php
...
<form method="POST">
  ...
  <div class="form-group">
    ...
    <input maxlength="30" type="text" class="form-control" id="titel" name="titel" aria-describedby="Titel lijst" placeholder="Vul hier je titel in">
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" id="beschrijving" name="beschrijving" aria-describedby="Vraag1" placeholder="Vul je beschrijving" maxlength="30">
